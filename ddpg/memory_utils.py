...
class Memory_Buffer(object):
    def __init__(self,thread_num,ob_space,ac_space,lock,size=10000,batch_size=64):
        ...
        self.rews_buffer = np.zeros(net_utils.combined_shape(size,()),dtype=np.float32)
        self.dones_buffer = np.zeros(net_utils.combined_shape(size,()),dtype=np.int32)

        self.current_thread = 0
        self.ptr = 0
        self.current_size = 0
    def store(self,s1,s2,act,rew,done):
        if self.lock.acquire():
            self.s1_buffer[self.ptr] = s1
            self.s2_buffer[self.ptr] = s2
            self.act_buffer[self.ptr] = act
            self.rews_buffer[self.ptr] = rew
