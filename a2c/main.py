...
from a2c_thread import Worker_Thread
...
def get_args():
    ...
    parser.add_argument("--nsteps",type=float,default=256)
    parser.add_argument("--batch_size",type=float,default=256*8)
    parser.add_argument("--nepochs",type=float,default=1)
    parser.add_argument("--ent_coef",type=float,default=0.01)
    parser.add_argument("--max_grad_norm",type=float,default=0.5)
    parser.add_argument("--train_vi_iters",type=int,default=20)
...
train_iters = int((args.nsteps * args.nepochs *args.thread_num) // args.batch_size)
lock = threading.Condition()
session = tf.InteractiveSession()
memory = mem_utils.Global_Memory(ob_space,ac_space,args.thread_num,train_iters)
...
for i in range(args.thread_num):
    threads.append(Worker_Thread(i,args.thread_num,make_env_fn,session,lock,memory,obs_normalizer,
                                 hidden_sizes,args.gamma,args.lam,args.nsteps,args.lr_rate,
                                 args.ent_coef,args.max_grad_norm,args.train_vi_iters,train_iters,args.total_timesteps))
